...
The code for the Tweeting process is incredibly straightforward (The example below will tweet whatever string "toTweet" contains.)  [Using the Twython library]
...
!http://i.imgur.com/7BJdJW4.png(I mean it....recognized three separate letters?...I guess that's good)!

Oh yeah.  We probably need to tweak some image settings!
I will spare you the details, just because they are truly a result of random tinkering.  I can only back up a few of my choices, and all else was purely brute force trial and error.
...
- Crop to the relevant parts (the board area)
- Blur
...
We have the contours, but we have to classify them.
...
All classifications were done from the same contour sets
However, after moving my rig to the kitchen and integrating onto the fridge, my opinion shifted.
As it turns out, deciphering a 640x480 resolution image from across the kitchen with tiny yellow letters in yellowish lighting on a slightly dirty yellowish fridge is tougher than first guess.
...
I read the initial contours as normal, and just changed my classifications after the fact to try and line up with associated letter colors
...
C, G, N, R, and X are the green letters in the magnet set we have, so my classifications are now narrowed to that range.  If I fail to classify as one of those options, the classifier attempts the next closest result in the 'neighbors' list (created by the initial classification attempt)
This conditional is repeated for each of the colors within my magnets.

This change improved my output so drastically that I could hardly believe it:

!http://i.imgur.com/QCiTlkq.png(The amount of joy in pure unadulterated squealing that I emitted when my computer outputted "CAT" was probably more than I should admit to)!
...
Many tweaks were made in this week.  Primarily adding a threshold for gibberish, as well as detection for when the lights were turned out at night.
...
This was a foolish plan.  Neither of the two webcams at my disposal have supported drivers on the Windows Server platform.  Logitech released a statement announcing their lack of intention to support Windows Server 2003. (I attempted to relocate this article for this post, but this <a href="http://forums.logitech.com/t5/Webcams/Which-Logitech-Webcam-runs-an-Windows-Server-2003/td-p/320761" target="_blank">unofficial response</a>) is the best I can do.
Regardless, after a solid week of getting absolutely no where, I am ashamed to say that the machines won.  I quit my attempts and returned to my regularly scheduled school assignments.

Some may call this quitting.
And I would be one of those people.

But who knows?  I may have just helped stave off the machine revolution for a tiny bit longer...

!http://i.imgur.com/a2Qtjek.png(I do not endorse the use of heavy appliances as self-aware entities)!

%{font-size:9pt}*For the sake of brevity, I neglected to mention that I also wrote a training program to provide the classifier's initial data associations.  Basically the trainer script takes the image, performs the image manipulations that I outlined above to get the contour objects, and then walks me through shape by shape, letting me enter the correct character. (Supervised learning)%

%{font-size:9pt}**I plan on putting the full script for both training and recognition on Github. I just have to be sure all the authorization tokens have been scrubbed. This code is incredibly useless for anyone not using my apartment kitchen, since most is just hardcoded values tuned to the environment and camera angle, but it could be fun to look at I suppose.%
